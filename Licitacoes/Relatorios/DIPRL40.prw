...
|=====================================================================================|
|Descrição | Contratos em aberto e envio por e-mail WF        |
|          |                                                                          |
|=====================================================================================|
|Sintaxe   | DIPRL40                                                                  |
|=====================================================================================|
...
Private tamanho    := "G"
Private limite     := 220
Private _cTitulo   := "RELAÇÃO DE CONTRATOS EM ABERTO"
Private _cDesc1    := ""
Private _cDesc2    := ""
...
Else
	/*==========================================================================\
	| Este relatório será executado via WORKFLOW todos os sábados
	\==========================================================================*/
	MV_PAR01 := cTod('')
	MV_PAR02 := dDataBase
	MV_PAR03 := cTod('')
...
Local _nTotGer   := 0   
Local _nTotEdi   := 0 
Local cAssunto   := EncodeUTF8("Licitação - Relatório "+If(MV_PAR16=2,"Analítico","Sintético")+" de Contratos em aberto  -  "+Alltrim(Posicione("SA3",1,xFilial("SA3")+MV_PAR05,"A3_NOME")),"cp1252")
Local cEmail     := Alltrim(Posicione("SA3",1,xFilial("SA3")+MV_PAR05,"A3_EMAIL"))
//Local cArqExcell := GetSrvProfString("STARTPATH","")+"Excell-DBF\"+U_DipUsr()+"-DIPRL40 - "+If(MV_PAR13=1,"Sintético","Analítico")
...
If MV_PAR16 = 1
	_cTitulo := "RELATÓRIO SINTÉTICO DE CONTRATOS EM ABERTO - Período:   "+dTOc(mv_par01)+' - '+dTOc(mv_par02)
	_cDesc1  := "EDITAL    PREGÃO                TIPO DO PREGÃO           DT. ASSIN.      VALIDADE            CLIENTE                                                                        VENDEDOR                             Valor Total"// Giovani Zago 22/08/11
	_cDesc2  := ""
Else
	_cTitulo := "RELATÓRIO ANALÍTICO DE CONTRATOS EM ABERTO - Período:   "+dTOc(mv_par01)+' - '+dTOc(mv_par02)
	_cDesc1  := "EDITAL    PREGÃO                TIPO DO PREGÃO           DT. ASSIN.      VALIDADE            CLIENTE                                                                        VENDEDOR"
	_cDesc2  := "Produto                                                                  Qtd. Vendida      Qtd. Entregue     Sal.Add.     Saldo       Valor unitário                                                             Valor Total"// Giovani Zago 22/08/11
Endif
...
Do While QRY1->(!Eof())
  ...
  If cWorkFlow == "N"
	...
	If MV_PAR17 = 1 //Giovani Zago 30/08/2011	
	    // ATUALIZANDO TABELA TEMPORÁRIA PARA GERAR ARQUIVO EM EXCEL	    	    
	    RecLock("TRB",.T.)                
	...
	TRB->DT_ASSIN  := CTOD(SUBSTRING(QRY1->UA1_ASSCON,7,2)+'/'+SUBSTRING(QRY1->UA1_ASSCON,5,2)+'/'+SUBSTRING(QRY1->UA1_ASSCON,1,4))//Giovani Zago 22/08/11
		...
		MsUnlock()
          ...
          EndIf
        // Início da impressão de relatório
		If li > 55
			If m_PAG > 1
	...
	Else // Sintético			    			   
		...
		// Atualiza Array para E-mail  sintético
		//If MV_PAR12 = 1
...
If cWorkFlow == "N"
	If MV_PAR16 = 1  
		If MV_PAR17 = 1 //Giovani Zago 30/08/2011
	// Alimentando tabela temporária para gerar excel do relatório sintético
	...
	// Envia e-mail com os dados do relatório
	If MV_PAR15 = 1 .And. MV_PAR05 == MV_PAR06 .And. !EMPTY(MV_PAR05)  // Só envia e´mail se o relatório for somente de um vendedor
	...
	ElseIf MV_PAR15 = 1 .And. MV_PAR05 <> MV_PAR06 .And. !EMPTY(MV_PAR06) 
		MsgInfo("Você selecionou vários vendedores...O e-mail só pode ser enviado para um vendedor por vez!")
	Endif
...
cMsg += '<tr>'
//cMsg += '<td width="100%" align="center"><font size="3" color="red">ATENÇÃO: Este e-mail deve ser respondido para Patrícia-Licitação</font></td>'
cMsg += '</tr>'
cMsg += '</table>'
...
cMsg += '<td align="Left" width="08%"><font  color="Black" size="3">PREGÃO</font></td>'
cMsg += '<td align="Left" width="15%"><font color="Black" size="3">TIPO PREGÃO</font></td>'
...
If MV_PAR16 = 2                  
	...
	cMsg += '<td align="Left" width="42%"><font size="2" color="Black" >Descrição</font></td>'
...
EndIF
	...
	ConOut("Enviando email dentro da função")
...
AADD(aRegs,{cPerg,"02","Encerramento Até        ?","","","mv_ch2","D",8,0,0,"G","","MV_PAR02","","","","","","","","","","","","","",""})
AADD(aRegs,{cPerg,"03","Assinatura de         ?","","","mv_ch3","D",8,0,0,"G","","MV_PAR03","","","","","","","","","","","","","",""})
AADD(aRegs,{cPerg,"04","Assinatura Até        ?","","","mv_ch4","D",8,0,0,"G","","MV_PAR04","","","","","","","","","","","","","",""})
AADD(aRegs,{cPerg,"05","Vendedor De        ?","","","mv_ch4","C",6,0,0,"G","","MV_PAR05","","","","","","","","","","","","","","","","","","","","","","","","",'SA3'})
AADD(aRegs,{cPerg,"06","Vendedor Até       ?","","","mv_ch5","C",6,0,0,"G","","MV_PAR06","","","","","","","","","","","","","","","","","","","","","","","","",'SA3'})
AADD(aRegs,{cPerg,"07","Fornecedor De      ?","","","mv_ch6","C",6,0,0,"G","","MV_PAR07","","","","","","","","","","","","","","","","","","","","","","","","",'SA2'})
AADD(aRegs,{cPerg,"08","Fornecedor Até     ?","","","mv_ch7","C",6,0,0,"G","","MV_PAR08","","","","","","","","","","","","","","","","","","","","","","","","",'SA2'})
AADD(aRegs,{cPerg,"09","Cliente De         ?","","","mv_ch8","C",6,0,0,"G","","MV_PAR09","","","","","","","","","","","","","","","","","","","","","","","","",'SA1'})
AADD(aRegs,{cPerg,"10","Cliente Até        ?","","","mv_ch9","C",6,0,0,"G","","MV_PAR10","","","","","","","","","","","","","","","","","","","","","","","","",'SA1'})
AADD(aRegs,{cPerg,"11","Produto De         ?","","","mv_cha","C",6,0,0,"G","","MV_PAR11","","","","","","","","","","","","","","","","","","","","","","","","",'SB1'})
AADD(aRegs,{cPerg,"12","Produto Até        ?","","","mv_chb","C",6,0,0,"G","","MV_PAR12","","","","","","","","","","","","","","","","","","","","","","","","",'SB1'})
AADD(aRegs,{cPerg,"13","Grupo de Clientes de?","","","mv_chc","C",6,0,0,"G","","MV_PAR13","","","","","","","","","","","","","","","","","","","","","","","","",'ACY'})
AADD(aRegs,{cPerg,"14","Grupo de Clientes até?","","","mv_chd","C",6,0,0,"G","","MV_PAR14","","","","","","","","","","","","","","","","","","","","","","","","",'ACY'})
AADD(aRegs,{cPerg,"15","Envia E-mail       ?","","","mv_che","N",1,0,1,"C","","MV_PAR15","Sim","","","","","Nao","","","","","","","",""})
AADD(aRegs,{cPerg,"16","Sintético/Analítico?","","","mv_chf","N",1,0,1,"C","","MV_PAR16","Sintético","","","","","Analítico","","","","","","","",""})
...
AADD(aRegs,{cPerg,"19","Edital até		   ?","","","mv_chi","C",6,0,0,"G","","MV_PAR19","","","","","","","","","","","","","","","","","","","","","","","","",'UA1'})
