...
±±ºPrograma  ³NOVO42    ºAutor  ³Microsiga           º Data ³  09/23/16   º±±
...
cCadastro := "Requisição de Produtos - PCP"
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³DIPA078   ºAutor  ³Microsiga           º Data ³  09/26/16   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
If !QRYSC2->(Eof())
	...
	EndDo
Else 
	Aviso("Atenção","Não existem OP's PREVISTAS para requisitar material",{"Ok"})
	lRet := .F.
EndIf              
QRYSC2->(dbCloseArea())
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³DIPA078   ºAutor  ³Microsiga           º Data ³  09/23/16   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
Local cMComSld:= ""
Local cMSemSld:= ""
Local cAssunto:= EncodeUTF8("REQUISIÇÃO PARCIAL - ","cp1252")
...
If Empty(nReqMat)
	Aviso("Atenção","Os produtos desta OP já estão disponíveis no armazém 99. Não será necessário incluir a requisição de materiais.",{"Ok"})
...
Else	       
	...
	For nI:=1 to Len(aMSemSld)
		If nI==1
			cMsgCic +="Produtos não requisitados: "+CHR(13)+CHR(10)
		EndIf
	...
	If Len(aMSemSld)>0 .Or. Len(aMComSld)>0                       
		...
		Aviso("Atenção",cMsgCic,{"Ok"},3)                         
	
		If (Aviso("Atenção","Requisição "+nReqMat+" gerada parcialmente. "+CHR(13)+CHR(10)+CHR(13)+CHR(10)+;
							"Clique em CONTINUAR para gerar a requisição e REMOVER a OP da lista de seleção."+CHR(13)+CHR(10)+CHR(13)+CHR(10)+;
							"OU"+CHR(13)+CHR(10)+CHR(13)+CHR(10)+;
							"Clique em FECHAR para gerar a requisição e MANTER a OP na lista de seleção.",{"CONTINUAR","FECHAR"},3)==1)			
...
If !Empty(cMsgCic)
	cMsgCic += CHR(13)+CHR(10)+"Usuário: "+AllTrim(U_DipUsr())
	U_DIPCIC("Requisição: "+nReqMat+CHR(13)+CHR(10)+CHR(13)+CHR(10)+cMsgCic,GetNewPar("ES_CICD078","DIEGO.DOMINGOS"))
	
	cMsgMail := "<html>"
	cMsgMail += " 	<head>"
	cMsgMail += " 	    <title>Requisição: "+nReqMat+"</title>"
	cMsgMail += " 	</head>"
	cMsgMail += " 	<body>"
	cMsgMail += " 	    <h2>Requisição: "+nReqMat+"</h2>"
	If Len(aMComSld)>0   		
		cMsgMail += "	<strong>Produtos requisitados parcialmente:</strong>"
		cMsgMail += cMComSld
	EndIf	
	If Len(aMSemSld)>0   	
		cMsgMail += "	<strong>Produtos não requisitados:</strong>"
		cMsgMail += cMSemSld
	EndIf
	cMsgMail += "		<br/>"
	cMsgMail += "		<br/>"
	cMsgMail += "		<strong>Usuário: "+AllTrim(U_DipUsr())+"</strong>"
	cMsgMail += "	</body>"
	cMsgMail += "</html>"
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³DIPA078   ºAutor  ³Microsiga           º Data ³  05/04/17   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
Else 
	Aviso("DIPA071","Produto não encontrado. Avise o T.I.",{"Ok"},1)
	lRet := .F.
EndIf            
