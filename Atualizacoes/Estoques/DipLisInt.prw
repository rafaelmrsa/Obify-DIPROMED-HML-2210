...
±±ºPrograma  ³DIPLISINT ºAutor  ³Microsiga           º Data ³  11/16/16   º±±
...
oDlg:bInit := {||EnchoiceBar(oDlg,{||oDlg:End()},{|| oDlg:End()},,)}
	...
	aItens1 := {'Código','Produto','Fornecedor'}        
	cCombo1 := aItens1[1]
	oCombo1 := TComboBox():New(35,02,{|u| IIf(PCount()>0,cCombo1:=u,cCombo1)},aItens1,50,20,oDlg,,,,,,.T.,,,,,,,,,'cCombo1')
	
	aItens2 := {'Igual','Contém'}
	cCombo2 := aItens2[1]
	oCombo2 := TComboBox():New(35,55,{|u| IIf(PCount()>0,cCombo2:=u,cCombo2)},aItens2,50,20,oDlg,,,,,,.T.,,,,,,,,,'cCombo2')	
...
cSQL := " 	FROM "
cSQL +=  		RetSQLName("SB1")
cSQL += "		INNER JOIN SB2010 ON B2_COD = B1_COD AND B2_FILIAL = B1_FILIAL AND B2_FILIAL = '"+xFilial("SB2")+"'"
cSQL += " 		WHERE
...
//Incluindo filtro por código do Fornecedor - MCVN 28/10/20
If !Empty(cFiltro)
	If oCombo1:nAt==1 //Código
		If oCombo2:nAt==1 //Igual
			cSQL += " 	B1_COD = '"+cFiltro+"' AND "
		Else //Contém
			cSQL += " 	B1_COD LIKE '%"+cFiltro+"%' AND "
		EndIf
	ElseIf oCombo1:nAt==2 //Descrição 
		If oCombo2:nAt==1 //Igual
			cSQL += " 	B1_DESC LIKE '"+cFiltro+"%' AND "	
		Else //Contém
			cSQL += " 	B1_DESC LIKE '%"+cFiltro+"%' AND "
...
oDlg := MSDialog():New(0,0,80,350,"Chave de Acesso",,,.F.,,,,,,.T.,,,.T. )
oDlg:bInit := {||EnchoiceBar(oDlg,{|| (nOpcao := 1,oDlg:End())},{|| (nOpcao := 0,oDlg:End())},,)}
	oSay :=	TSay():New(12,15,{|| OemToAnsi("Chave: ")},oDlg,,,,,, .T.,,, 250,100,,,,,,)
	oGet := TGet():New(10,35,{|u| IIf(PCount()>0,cTexto:=u,cTexto)},oDlg,130,009,"@!",,,,,,,.T.,,,,,,,,,,"cTexto",,,, )	
oDlg:Activate(,,,.T.)	          
