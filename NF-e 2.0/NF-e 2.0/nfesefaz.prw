...
//***************************************************************************
//TODAS AS ALTERAÇÕES NECESSÁRIAS NESSE FONTE PODERÃO SER LOCALIZADAS 
//UTILIZANDO-SE A PALAVRA-CHAVE "DIPROMED"
//***************************************************************************
...
//********************************************************************
//DIPROMED - Mensagem especifica para desoneração
//******************************************************************** 
Private _lDipDeson 	:= .F. //MCVN 02/04/2011
Private _nDipVlPro	:= 0
//********************************************************************
					...
					If FunName() == "SPEDNFSE" //.Or. FunName() == "SPEDCTE"
						...
						If SF3->F3_TIPO =="S"
							aadd(aISSQN,;
										...
										SF3->F3_CNAE,;
										SF3->F3_ALIQICM,;
										IIf((RetEndEntM0("CodMun") == "3106200" .And. cTpPessoa == "EP"),nValIss,SF3->F3_VALICM),;
										SF3->F3_VALOBSE,;
										cTribMun,;
										SF3->F3_BASEICM,;
										cSitTrib})
			...
			Else
				...
				If !SF2->F2_TIPO $ "DB" 
					...
					*/
					If !lUsaCliEnt
						...
						If !Empty(AllTrim(SF2->F2_CLIENT)) .And. !Empty(AllTrim(SF2->F2_LOJENT))			
							If Len(aEntrega) > 0											
								...
								//If aEntrega[08] <> IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) .And. !lCNPJIgual //aEntrega[08] <> Upper(SA1->A1_EST) //Rafael Moraes Rosa - 12/01/2023 - LINHA COMENTADA
								If aEntrega[08] <> IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) .And. !lCNPJIgual //aEntrega[08] <> Upper(SA1->A1_EST) //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
								...
								//If aEntrega[08] == IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) .OR. lCNPJIgual //Rafael Moraes Rosa - 12/01/2023 - LINHA COMENTADA
								If aEntrega[08] == IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) .OR. lCNPJIgual //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
									SA1->(MsSeek(xFilial("SA1")+SF2->F2_CLIENTE+SF2->F2_LOJA))
								EndIf
							Endif
						Else
					...
					aadd(aDest,AllTrim(SA1->A1_DDD)+SA1->A1_TEL)                                                 				
					If !Upper(SA1->A1_EST) == "EX"                                                      				
						//If !Empty(SA1->A1_INSCRUR) .And. SA1->A1_PESSOA == "F" .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "PR"  .And. SA1->A1_EST == "PR" //Rafael Moraes Rosa - 12/01/2023 - LINHA COMENTADA
						If !Empty(SA1->A1_INSCRUR) .And. SA1->A1_PESSOA == "F" .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) == "PR"  .And. SA1->A1_EST == "PR" //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
				...
				If !Empty(SF2->F2_TRANSP)
					...
					//unidade da Federação e a inscrição estadual do transportador (CHAMADO-TVMWZL).
						//if (IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "MG") .and. Empty(SA4->A4_INSEST) .and. (ALLTRIM(Upper(SA4->A4_NOME)) =='REMETENTE' .OR. ALLTRIM(Upper(SA4->A4_NOME)) =='DESTINATARIO') //Rafael Moraes Rosa - 12/01/2023 - LINHA COMENTADA
						if (IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) == "MG") .and. Empty(SA4->A4_INSEST) .and. (ALLTRIM(Upper(SA4->A4_NOME)) =='REMETENTE' .OR. ALLTRIM(Upper(SA4->A4_NOME)) =='DESTINATARIO') //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
				...
				If MsSeek(cChave)
					...
					If AllTrim(SF3->F3_CFO) == "5111" .Or. AllTrim(SF3->F3_CFO) == "6111" .or.  AllTrim(SF3->F3_CFO)  == '5918' .or.  AllTrim(SF3->F3_CFO)  == '6918'
						lConsig  := .T.
					//elseif ( AllTrim(SF3->F3_CFO) == "5949" .or. AllTrim(SF3->F3_CFO) == "5910" ) .and. SM0->M0_ESTENT == 'SP' /*termos do inciso II do art. 456 do RICMS/ SP  chamado THPXGS*/ //Rafael Moraes Rosa - 12/01/2023 - LINHA COMENTADA
					elseif ( AllTrim(SF3->F3_CFO) == "5949" .or. AllTrim(SF3->F3_CFO) == "5910" ) .and. RetEndEntM0("Estado") == 'SP' /*termos do inciso II do art. 456 do RICMS/ SP  chamado THPXGS*/ //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
				...
				#ENDIF
                //------------------------------------------------------------------------------------------------
			    // DIPROMED - Quando a nota fiscal possuir mais que uma natureza, deve-se concatena-las e mostrar
			    // ate o limite de 50 caracteres.
                //------------------------------------------------------------------------------------------------				
				cNatOper  	:= ""   // Imprimir Multiplas naturezas - MCVN 02/04/2011           
				cSeparador	:= ""   // Separador de naturezas - MCVN 02/04/2011
				//------------------------------------------------------------------------------------------------
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					//If (cAliasSD2)->D2_TIPO == "D" .And. SM0->M0_ESTENT == "PR" .And. (cAliasSD2)->D2_ICMSRET > 0 //Rafael Moraes Rosa - 12/01/2023 - LINHA COMENTADA
					If (cAliasSD2)->D2_TIPO == "D" .And. RetEndEntM0("Estado") == "PR" .And. (cAliasSD2)->D2_ICMSRET > 0 //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
	          ...
	          // Posiciono na TES do cupom fiscal para pegar a natureza de operação da nf sobre cupom
						   ...
						   Endif	
							...
							If !lNatOper
								...
								*/
 								...
 								If !AllTrim(SF4->F4_TEXTO) $ cNatOper .AND. (Len(cNatOper) + Len(AllTrim(SF4->F4_TEXTO))) < 51  //MCVN 02/04/2011
							         cNatOper 	+= cSeparador + AllTrim(SF4->F4_TEXTO) 											//MCVN 02/04/2011
							         cSeparador	:= ' / '																		//MCVN 02/04/2011
							    EndIf																							//MCVN 02/04/2011
							    //------------------------------------------------------------------------------------------------     
									...
									If AllTrim(SF1->F1_ESPECIE)=="NFP"
										...
										If SD1->D1_TIPO$"DB"
												...
												*/
												...
												aadd(aNfVincRur,{SD1->D1_EMISSAO,SD1->D1_SERIE,SD1->D1_DOC,SF1->F1_ESPECIE,;
												IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA1->A1_CGC),; 
												IIF(SD1->D1_FORMUL=="S",RetEndEntM0("Estado"),SA1->A1_EST),; 
												IIF(SD1->D1_FORMUL=="S",SM0->M0_INSC,SA1->A1_INSCR)})
										...
										Else
											...
											*/
											...
											aadd(aNfVincRur,{SD1->D1_EMISSAO,SD1->D1_SERIE,SD1->D1_DOC,SF1->F1_ESPECIE,;
											IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA2->A2_CGC),; 
											IIF(SD1->D1_FORMUL=="S",RetEndEntM0("Estado"),SA2->A2_EST),; 
											IIF(SD1->D1_FORMUL=="S",SM0->M0_INSC,SA2->A2_INSCR)})
											//Rafael Moraes Rosa - 12/01/2023 - FIM - LINHA SUBSTITUIDA
								...
								ElseIf (cAliasSD2)->D2_TIPO == "N"                                                     						
									...
									dbSelectArea("SFT")
							   		...
							   		If MsSeek(xFilial("SFT")+"S"+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_NFORI)
										...
										If SFT->FT_ESTADO == "EX" .or. ((SubStr(RetEndEntM0("CodMun"),1,2) == "35" .Or. SubStr(RetEndEntM0("CodMun"),1,2) == "29") .and. "REMESSA POR CONTA E ORDEM DE TERCEIROS" $ Upper(cNatOper) .and. lOrgaoPub )//(Venda para orgao publico - SP/BA/CFOP Remessa por conta e ordem de terceiros (cfop 5923/6923)- ch:TIDWCY   
									   		...
									   		If MsSeek(xFilial("SF3")+SFT->FT_CLIEFOR+SFT->FT_LOJA+SFT->FT_NFISCAL+SFT->FT_SERIE)										   		
												...
												If AllTrim(SF3->F3_ESPECIE)=="NFP"
													...
													*/
													...
													aadd(aNfVincRur,{SF3->F3_EMISSAO,SF3->F3_SERIE,SF3->F3_NFISCAL,SF3->F3_ESPECIE,;
														SM0->M0_CGC,RetEndEntM0("Estado"),SM0->M0_INSC})
						...
						cModFrete := IIF(SC5->C5_TPFRETE$"F/R","1","0")   // MCVN 02/04/2011 //DIPROMED

						if !lIcmDevol .And. !("Nota fiscal emitida sem destaque do ICMS" $ cMensCli)
							if Len( cMensCli ) > 0
								cMensCli += ' '
							endif
							//if SM0->M0_ESTENT == "PR" //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
							if RetEndEntM0("Estado") == "PR" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
						...
						//Verifica se existe Template DCL
			     		...
			     		//*********************************************************************
			     		// DIPROMED - Alterações no tipo de frete
			     		//*********************************************************************
		     			If SF2->F2_TPFRETE$"CI" // MCVN 02/04/2011
							cModFrete := "0"
						...
						ElseIf SF2->F2_TPFRETE == "S"
					 	...
					 	ElseIf Empty(cModFrete)
					 		...
					 		// DIPROMED - Alterações no tipo de frete
					 		//*********************************************************************					 	
					 		If SC5->C5_TPFRETE$"CI" // MCVN 02/04/2011
								cModFrete := "0"
						...
						//If SFT->(FieldPos("FT_DS43080")) <> 0 .And. SFT->FT_DS43080 > 0 .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "MG" //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
						If SFT->(FieldPos("FT_DS43080")) <> 0 .And. SFT->FT_DS43080 > 0 .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) == "MG" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
						...
						lCodLan := .F.
						//If SM0->M0_ESTENT $ "PR/RJ/RS/" //TAG cBenef buscar o conteúdo da tabela 5.2 no sistema quando for do PR. //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
						If RetEndEntM0("Estado") $ "PR/RJ/RS/" //TAG cBenef buscar o conteúdo da tabela 5.2 no sistema quando for do PR. //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
							...
							If MsSeek(xFilial("CDV") +'S'+PadR('SPED',TamSX3("CDV_ESPECI")[1])+'S'+(cAliasSD2)->(D2_DOC+D2_SERIE+D2_CLIENTE+D2_LOJA+D2_ITEM))
								...
								//cCodlan := getCodLan( alltrim(SM0->M0_ESTENT), SF4->F4_SITTRIB, cAmbiente ) //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
								cCodlan := getCodLan( alltrim(RetEndEntM0("Estado")), SF4->F4_SITTRIB, cAmbiente ) //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA

							EndIF
						Else
							cCodlan := ""	
							//If SM0->M0_ESTENT <> "SP" .and. CDA->(ColumnPos("CDA_CODLAN")) > 0 //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
							If RetEndEntM0("Estado") <> "SP" .and. CDA->(ColumnPos("CDA_CODLAN")) > 0 //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
								dbSelectArea("CDA")
								dbSetOrder(1)
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							IIf(SubStr(RetEndEntM0("CodMun"),1,2) == "35" .And. cTpPessoa == "EP" .And. nDescIcm > 0, nDescIcm,0),;   
						...
						While CD2->(!Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
							...
							//ElseIf SFT->(FieldPos("FT_TRFICM")) <> 0 .And. SFT->FT_TRFICM <> 0 .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) $ "RS/GO" //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
							ElseIf SFT->(FieldPos("FT_TRFICM")) <> 0 .And. SFT->FT_TRFICM <> 0 .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) $ "RS/GO" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
							...
							//If SFT->(FieldPos("FT_PR43080")) <> 0 .And. SFT->FT_PR43080 <> 0 .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "MG" //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
							If SFT->(FieldPos("FT_PR43080")) <> 0 .And. SFT->FT_PR43080 <> 0 .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) == "MG" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
							...
							Do Case
								...
								//********************************************************************
								//DIPROMED - Mensagem especifica para desoneração - MCVN 02/04/2011
								//********************************************************************
								If SD2->D2_TES$GetNewPar("ES_TESDESO","764")
									_lDipDeson 		:= .T.
									_nDipVlPro 		+= CD2->CD2_BC / ((100 - CD2->CD2_ALIQ) / 100)
									aTail(aICMS) 	:= {CD2->CD2_ORIGEM,;
														POSICIONE("SF4", 1, xFilial("SF4") + cD2Tes, "F4_SITTRIB"),;
														"",;
														0,;
														CD2->CD2_BC / ((100 - CD2->CD2_ALIQ) / 100),;
														CD2->CD2_ALIQ,;
														(CD2->CD2_BC / ((100-CD2->CD2_ALIQ) / 100)) * (CD2->CD2_ALIQ / 100),;
														0,;
														CD2->CD2_QTRIB,;
														CD2->CD2_PAUTA,;
														POSICIONE("SF4", 1, xFilial("SF4") + cD2Tes, "F4_MOTICMS"),;
														0,;
														"1",;
														"2",;
				...
				If ((SubStr(RetEndEntM0("CodMun"),1,2) == "35" ) .and. "REMESSA POR CONTA E ORDEM DE TERCEIROS" $ Upper(cNatOper) .and. lOrgaoPub )
...
Else
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		Else				
			...
			If SF1->F1_TIPO $ "DB" 
				...
				*/
				...
				If cDevMerc == "S"
					...
					//aadd(aDest,IIF(!lEndFis,ConvType(FisGetEnd(SM0->M0_ENDCOB,SM0->M0_ESTCOB)[1]),ConvType(FisGetEnd(SM0->M0_ENDENT,SM0->M0_ESTENT)[1])))// 3 //Rafael Moraes Rosa - 12/01/2023 - LINHA COMENTADA
					aadd(aDest,IIF(!lEndFis,ConvType(FisGetEnd(SM0->M0_ENDCOB,SM0->M0_ESTCOB)[1]),ConvType(FisGetEnd(RetEndEntM0("Endereco"),RetEndEntM0("Estado"))[1])))// 3 //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
					...
					Else
						...
						*/
						...
						//Rafael Moraes Rosa - 12/01/2023 - INICIO - LINHA SUBSTITUIDA
						If FisGetEnd(RetEndEntM0("Endereco"),RetEndEntM0("Estado"))[2] <> 0
							aadd(aDest,FisGetEnd(RetEndEntM0("Endereco"),RetEndEntM0("Estado"))[3])// 4
						//Rafael Moraes Rosa - 12/01/2023 - FIM - LINHA SUBSTITUIDA
					...
					//Rafael Moraes Rosa - 12/01/2023 - INICIO - LINHA COMENTADA
					/*
					cEndEmit :=  IIF(!lEndFis,Iif(!Empty(SM0->M0_COMPCOB),SM0->M0_COMPCOB,ConvType(FisGetEnd(SM0->M0_ENDCOB,SM0->M0_ESTCOB)[4]) ) ,Iif(!Empty(SM0->M0_COMPENT),SM0->M0_COMPENT,ConvType(FisGetEnd(SM0->M0_ENDENT,SM0->M0_ESTENT)[4]) ) )
					aadd(aDest,cEndEmit)// 5
	
					aadd(aDest,IIF(!lEndFis,ConvType(SM0->M0_BAIRCOB),ConvType(SM0->M0_BAIRENT)))// 6
					
					aadd(aDest,ConvType(RetEndEntM0("CodMun")))// 7
					...
					*/
					...
					//Rafael Moraes Rosa - 12/01/2023 - INICIO - LINHA SUBSTITUIDA
					cEndEmit :=  IIF(!lEndFis,Iif(!Empty(SM0->M0_COMPCOB),SM0->M0_COMPCOB,ConvType(FisGetEnd(SM0->M0_ENDCOB,SM0->M0_ESTCOB)[4]) ) ,Iif(!Empty(RetEndEntM0("Compl")),RetEndEntM0("Compl"),ConvType(FisGetEnd(RetEndEntM0("Endereco"),RetEndEntM0("Estado"))[4]) ) )
					aadd(aDest,cEndEmit)// 5
	
					aadd(aDest,IIF(!lEndFis,ConvType(SM0->M0_BAIRCOB),ConvType(RetEndEntM0("Bairro"))))// 6
					
					aadd(aDest,ConvType(RetEndEntM0("CodMun")))// 7
					aadd(aDest,IIF(!lEndFis,ConvType(SM0->M0_CIDCOB),ConvType(RetEndEntM0("Cidade"))))// 8				
					
					aadd(aDest,Upper(IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado")))))// 9
					aadd(aDest,IIF(!lEndFis,ConvType(SM0->M0_CEPCOB),ConvType(RetEndEntM0("CEP"))))// 10
					//Rafael Moraes Rosa - 12/01/2023 - FIM - LINHA SUBSTITUIDA
					...
					If !lUsaCliEnt
						...
						If !Empty(AllTrim(SF1->F1_FORENT)) .And. !Empty(AllTrim(SF1->F1_LOJAENT))			
							If Len(aEntrega) > 0											
								...
								//tenho que buscar os dados do cliente de entrega para nao ocorrer 
								//rejeicao 523 - CFOP não é de Operação Estadual e UF emitente igual à UF destinatário
								//If aEntrega[08] <> IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) .And. !lCNPJIgual //aEntrega[08] <> Upper(SA1->A1_EST) //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
								If aEntrega[08] <> IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) .And. !lCNPJIgual //aEntrega[08] <> Upper(SA1->A1_EST) //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
									MsSeek(xFilial("SA1")+SF1->F1_FORENT+SF1->F1_LOJAENT)		
								EndIf
								//Se a UF de entrega for igual a UF do emitente (Operação interna) - busco os dados do cliente para montar como destinatario.
								//Se o CNPJ do emitente for igual ao do destinatário também levo os dados do cliente, mesmo que UFs forem diferente.
								//Se o cliente não for consumidor final e possuir IE, pode ocorrer a rejeição 773 - Operação Interna e UF de destino difere da UF do emitente
								//If aEntrega[08] == IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) .OR. lCNPJIgual //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
								If aEntrega[08] == IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) .OR. lCNPJIgual //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
									MsSeek(xFilial("SA1")+SF1->F1_FORNECE+SF1->F1_LOJA)
								EndIf
							Endif
						Else
		    ...
		    // ate o limite de 50 caracteres.
	        //------------------------------------------------------------------------------------------------			
			cNatOper  := "" // MCVN 02/04/2011 - Imprimir Multiplas naturezas           
			cSeparador:= "" // MCVN 02/04/2011 - Separador de naturezas
			//------------------------------------------------------------------------------------------------
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
	    		...
	    		If (SF4->(FieldPos("F4_PICMDIF"))>0 .And. "66.66" $ Alltrim(Str(SF4->F4_PICMDIF)) ) ;
	    			...
	    			.And. (SubStr(RetEndEntM0("CodMun"),1,2)=='41' .And. SubStr((cAliasSD1)->D1_CF,1,1) == '3')	    			
					lDifParc := .T.
	    		EndIf
                ...
                RetInfoSBZ((cAliasSD1)->D1_COD, @cUmDipi, @nConvDip)
	     		...
	     		//*********************************************************************				
				If SF1->F1_FRETEPO <> '' 
				    cModFrete := IIF(SF1->F1_FRETEPO $ "CNI","0","1") 	//Tratamendo para determinar o tipo de Frete - MCVN 02/04/2011                                         
				Else					
				    cModFrete := IIF(SF1->F1_FRETE > 0,"0","1")			//Tratamendo para determinar o tipo de Frete - MCVN 02/04/2011                                         
				EndIf
				//*********************************************************************
...
IF lPe01Nfe     
	//****************************************************************************************
	// DIPROMED - adicionada as variáveis "cClieFor" e "cLoja" para possibilitar a pesquisa 
	// de cliente/fornecedor dentro do ponto de entrada PE01NFESEFAZ
	// RODRIGO TELECIO EM 25/07/2018
	//****************************************************************************************
...
If cVeramb >= "3.10"
	...
	If cOper == "3" .And. Len(aComb) > 0 .And. aDest[9] != "EX" .And. aComb[1][4] == "EX"		
		//If aDest[9] == IIF(!GetNewPar("MV_SPEDEND",.F.),ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
		If aDest[9] == IIF(!GetNewPar("MV_SPEDEND",.F.),ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
			cOper:= "1" 
		Else
			cOper:= "2"
		EndIf		
...
cString += '<enderEmit>'
//cString += '<Lgr>'+IIF(!lEndFis,ConvType(FisGetEnd(SM0->M0_ENDCOB,SM0->M0_ESTCOB)[1]),ConvType(FisGetEnd(SM0->M0_ENDENT,SM0->M0_ESTENT)[1]))+'</Lgr>' //Rafael Moraes Rosa - 12/01/2023 - LINHA COMENTADA
cString += '<Lgr>'+IIF(!lEndFis,ConvType(FisGetEnd(SM0->M0_ENDCOB,SM0->M0_ESTCOB)[1]),ConvType(FisGetEnd(RetEndEntM0("Endereco"),RetEndEntM0("Estado"))[1]))+'</Lgr>' //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
...
Else
	...
	*/
	...
	//Rafael Moraes Rosa - 12/01/2023 - INICIO - LINHA SUBSTITUIDA
	If FisGetEnd(RetEndEntM0("Endereco"),RetEndEntM0("Estado"))[2]<>0
		cString += '<nro>'+FisGetEnd(RetEndEntM0("Endereco"),RetEndEntM0("Estado"))[3]+'</nro>'
	//Rafael Moraes Rosa - 12/01/2023 - FIM - LINHA SUBSTITUIDA
...
cString += '<cMun>'+ConvType(RetEndEntM0("CodMun"))+'</cMun>'
...
cEndEmit :=  IIF(!lEndFis,Iif(!Empty(SM0->M0_COMPCOB),SM0->M0_COMPCOB,ConvType(FisGetEnd(SM0->M0_ENDCOB,SM0->M0_ESTCOB)[4]) ) , Iif(!Empty(RetEndEntM0("Compl")),RetEndEntM0("Compl"),ConvType(FisGetEnd(RetEndEntM0("Endereco"),RetEndEntM0("Estado"))[4]) ) )
cString += NfeTag('<Cpl>',cEndEmit)
cString += '<Bairro>'+IIF(!lEndFis,ConvType(SM0->M0_BAIRCOB),ConvType(RetEndEntM0("Bairro")))+'</Bairro>'
cString += '<cMun>'+ConvType(RetEndEntM0("CodMun"))+'</cMun>'
cString += '<Mun>'+IIF(!lEndFis,ConvType(SM0->M0_CIDCOB),ConvType(RetEndEntM0("Cidade")))+'</Mun>'
cString += '<UF>'+IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado")))+'</UF>'
cString += NfeTag('<CEP>',IIF(!lEndFis,ConvType(SM0->M0_CEPCOB),ConvType(RetEndEntM0("CEP"))))
//Rafael Moraes Rosa - 12/01/2023 - FIM - LINHA SUBSTITUIDA
...
If !Empty(aIEST) 
	...
	*/
	...
	//Rafael Moraes Rosa - 13/01/2023 - INICIO - LINHA SUBSTITUIDA
	If((AllTrim(ConvType(VldIE(SM0->M0_INSC))) <> Alltrim(aIEST[01])) .And. (Alltrim (aIEST[01]) <> Alltrim(aIEST[02])) .And.;
			 (cIdDest == "2" .OR. (cIdDest == "1" .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) == "PR")))
	//Rafael Moraes Rosa - 13/01/2023 - FIM - LINHA SUBSTITUIDA
...
Static Function NfeDest(aDest,cVerAmb,aTransp,aCST,lBrinde,cMunDest)
	...
	cVerAmb	:= PARAMIXB[2] 
	cMunDest	:= iIf(!lBrinde,IIf(Len(aDest[07])>5,aDest[07],aUF[aScan(aUF,{|x| x[1] == aDest[09]})][02]+aDest[07]),RetEndEntM0("CodMun"))
	...
	cString += '<enderDest>'
	//cString += '<Lgr>'+ConvType(iIf(!lBrinde,aDest[03],(FisGetEnd(SM0->M0_ENDENT,SM0->M0_ESTENT)[1])))+'</Lgr>' //Rafael Moraes Rosa - 12/01/2023 - LINHA COMENTADA
	cString += '<Lgr>'+ConvType(iIf(!lBrinde,aDest[03],(FisGetEnd(RetEndEntM0("Endereco"),RetEndEntM0("Estado"))[1])))+'</Lgr>' //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
	...
	*/
	...
	cString += NfeTag('<Cpl>',ConvType(iIf(!lBrinde,aDest[05],RetEndEntM0("Compl"))))
	cString += '<Bairro>'+ConvType(iIf(!lBrinde,aDest[06],RetEndEntM0("Bairro")))+'</Bairro>'
	cString += '<cMun>'+ConvType(cMunDest)+'</cMun>'
	cString += '<Mun>'+ConvType(iIf(!lBrinde,aDest[08],RetEndEntM0("Cidade")))+'</Mun>'
	cString += '<UF>'+ConvType(iIf(!lBrinde,aDest[09],RetEndEntM0("Estado")))+'</UF>'
	cString += NfeTag('<CEP>',iIf(!lBrinde,aDest[10],RetEndEntM0("CEP")))
...
Local lEndFis 		:= GetNewPar("MV_SPEDEND",.F.)
local lDateRefNf	:= .T.
Local aPPDifal		:= &(SuperGetMV("MV_PPDIFAL", ,"{{2016,40,60},{2017,60,40},{2018,80,20},{2019,100,0},{2020,100,0}}"))
Local aPICMSInter	:= {}
Local nX			:= 0
Local nBaseIcm   	:= 0
Local nValCof 		:= 0
	...
	Else
		...
		If Len(aIcms)>0
			...
			ElseIf ( aCST[1] == '51' .And. lArt186 )
				//If !Empty(aICMS[12]) .and. !Empty(aICMS[07]) .and. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) $ "RS" //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
				If !Empty(aICMS[12]) .and. !Empty(aICMS[07]) .and. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) $ "RS" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
			...
			//If cVerAmb >= "3.10" .and. aCST[1] $ '10' .and. (IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "PR") .and. !Empty(aICMS[12]) //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
			If cVerAmb >= "3.10" .and. aCST[1] $ '10' .and. (IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) == "PR") .and. !Empty(aICMS[12]) //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
		...
		If (aCST[01] = "60" .or. cCsosn$ "500")		
			...
			If nBaseIcm > 0 .and. nValICM > 0	.and. nAlqICM > 0
		   		If Len(cMensFis) > 0 .And. SubStr(cMensFis, Len(cMensFis), 1) <> " "
					cMensFis += " "
				EndIf
			    //If cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "SP" //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
				If cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) == "SP" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
				...
				//ElseIF cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "PR" //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
				ElseIF cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) == "PR" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
					...
					*/
				//ElseIF cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "SC" //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
				ElseIF cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) == "SC" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA  
					...
					EndIf	 	
				//ElseIF cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "AM" //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
				ElseIF cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) == "AM" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
				 	lNContrICM := IIf(Empty(SA1->A1_INSCR) .Or. "ISENT"$SA1->A1_INSCR .Or. "RG"$SA1->A1_INSCR .Or. ( SA1->(FieldPos("A1_CONTRIB"))>0 .And. SA1->A1_CONTRIB == "2"),.T.,.F.)
				...
				//ElseIF cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "RS" //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA	
				ElseIF cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) == "RS" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA		 
				 	if !Empty(aProd[28])				 	
				 	...
				 	endIf					 	
				//ElseIf cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "ES" .And. Len(aICMS) > 0 .And. ( nBaseIcm+nValICM > 0 ) //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
				ElseIf cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) == "ES" .And. Len(aICMS) > 0 .And. ( nBaseIcm+nValICM > 0 ) //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
				...
				//ElseIf cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "MG" .And. Len(aICMS) > 0 .And. ( nBaseIcm+nValICM > 0 )  // Conforme Chamado TIABCS //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
				ElseIf cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) == "MG" .And. Len(aICMS) > 0 .And. ( nBaseIcm+nValICM > 0 )  // Conforme Chamado TIABCS //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
				...
				//ElseIf IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "SP" //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA 
				ElseIf IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) == "SP" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA 
					If cIndFinal == "1"
				...
				//ElseIf IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "RJ" //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
				ElseIf IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) == "RJ" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
...
Else
	If Len(aISSQN)>0 .and. !Empty(aISSQN[01])
		...
		cString += '</Tributo>'
		cString += '<cpl>'
		cString += '<cmunfg>'+ConvType(RetEndEntM0("CodMun"))+'</cmunfg>'	
...
//If nIcmsDif > 0 .And. aDest[9] == 'PR' .And. aCST[1] $ '10' .And. SM0->M0_ESTENT == 'PR' //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
If nIcmsDif > 0 .And. aDest[9] == 'PR' .And. aCST[1] $ '10' .And. RetEndEntM0("Estado") == 'PR' //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
...
If cVerAmb == "4.00"
	...
	If cIdDest == "2" .or. (cIdDest == "1" .And. RetEndEntM0("CodMun") <> cMunDest .And. !lMVINTTRAN)
		lGeraTags:= .F.  
	Endif
...
//DIPROMED - Mensagem especifica para desoneração
//********************************************************************
Local _nDipTotNF:= 0 //MCVN 02/04/2011
//********************************************************************
Local cString   		:= ""
Local cCfor     		:= ""
Local cLojaEn   		:= ""
...
If Len(cMsgCli)>0 .and. !Empty(cMsgCli)
	...
	//IF (SM0->M0_ESTENT) $ "SP" .AND. cTipo=='0' .And. !Empty(cSerRefcup + cNfRefcup) //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
	IF (RetEndEntM0("Estado")) $ "SP" .AND. cTipo=='0' .And. !Empty(cSerRefcup + cNfRefcup) //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
		SFT->(dbSetOrder(6))
...
//if SM0->M0_ESTENT == 'SP' .and. Len(aNfVinc) > 0 //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
if RetEndEntM0("Estado") == 'SP' .and. Len(aNfVinc) > 0 //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
...
If Len( aNfVinc ) > 0 //Nota de espécie NFE ou NCE ou CTE vinculada
	For nZ := 1  to Len( aNfVinc )
		If !( aNfVinc[nZ][2] + aNfVinc[nZ][3] ) $ cChvNFe
			...
			Else
				//if SM0->M0_ESTENT == 'SP' //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
				if RetEndEntM0("Estado") == 'SP' //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
			...
			//if SM0->M0_ESTENT == 'SP' //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
			if RetEndEntM0("Estado") == 'SP' //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA 
...
Next
If nValII > 0
	//If lEasy .And. IIF(!GetNewPar("MV_SPEDEND",.F.),ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "SP" .and. lEIC0064 //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
	If lEasy .And. IIF(!GetNewPar("MV_SPEDEND",.F.),ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) == "SP" .and. lEIC0064 //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
...
If _lDipDeson
	For nI:=1 To Len(aProd)
		_nDipTotNF += aProd[nI,10]
	Next nI
	cString += "Preco com ICMS: " + ConvType(_nDipVlPro,15,2) + " - Desconto Isencao ICMS: " + ConvType(_nDipVlPro-_nDipTotNF,15,2) + " - Preco total sem ICMS: " + ConvType(_nDipTotNF,15,2)
//********************************************************************

//Tratamento para adcionar o valor do ICMS desonerado para informação complementar da Danfe.
ElseIf nVicmsDeson >0
...
If nvFCPUFDest > 0 .or.  nvICMSUFDest > 0 .or. nvICMSUFRemet > 0 .or. nvBCUFDest  > 0     
 	//IF (IIF(!(GetNewPar("MV_SPEDEND",.F.)),ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "BA" ) //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
	IF (IIF(!(GetNewPar("MV_SPEDEND",.F.)),ConvType(SM0->M0_ESTCOB),ConvType(RetEndEntM0("Estado"))) == "BA" ) //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA  
...
//Valor Líquido
If RetEndEntM0("CodMun") == "3106200" .And. cTpPessoa == "EP"  // Tratamento realizado para o municipio de Belo Horizonte- MG quando o Tomador for Órgão Público
...
If "1"$cModXml //BH - ABRASF
	...
	If RetEndEntM0("CodMun") == "3106200" .And. cTpPessoa == "EP"
...
If AllTrim(cMunPres)==AllTrim(RetEndEntM0("CodMun"))
	cInscMun:=aDest[11]
EndIf
...
Do Case
	Case (!Empty(aNfVinc) .And. !(aNota[5]$"NDB") .And. SF4->F4_AJUSTE <> "S")
  		cTPNota:= "2" 
 	Case (SubStr(RetEndEntM0("CodMun"),1,2)=='31' .And. SF4->F4_AJUSTE == "S" .And. (aNota[5]) $ "N" )
 		cTPNota:= "3"
...
±±ºPrograma  ³ RetEndEntM0     ºAutor  ³Rafael Rosa    º Data ³  12/01/23 º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Valida e retorna o Endereco de Entrega valido.		      º±±
±±º          								                              º±±
±±ÌÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso     ³ Especifico DIPROMED                                          º±±
±±         ³ ³ Sua finalidade visa retornar o endereco de entrega da      º±±
±±         ³ ³ filial 0104 caso a recorrente logada corresponda a 0101.   º±±
±±             													          º±±
...
Static Function RetEndEntM0(cRecEndEnt)
...
IF cEmpAnt+cFilAnt = '0101'
	...
	SM0->(dbSeek("0104"))
		...
		cEnvEndEnt	:=	IIF(	cRecEndEnt="Endereco",	SM0->M0_ENDENT,;
						IIF(	cRecEndEnt="CodMun",	SM0->M0_CODMUN,;
						IIF(	cRecEndEnt="Cidade",	SM0->M0_CIDENT,;
						IIF(	cRecEndEnt="Estado",	SM0->M0_ESTENT,;
						IIF(	cRecEndEnt="CEP",		SM0->M0_CEPENT,;
						IIF(	cRecEndEnt="Bairro",	SM0->M0_BAIRENT,SM0->M0_COMPENT))))))
