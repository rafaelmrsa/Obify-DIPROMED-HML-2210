...
#DEFINE HMARGEM    030
#DEFINE VMARGEM    030
#DEFINE MAXITEM   028                                                // Máximo de produtos para a primeira página
#DEFINE MAXITEMP2 058                                                // Máximo de produtos para as páginas adicionais
...
#DEFINE MAXITEMC  044                                                // Máxima de caracteres por linha de produtos/serviços
#DEFINE MAXMENLIN 095                                                // Máximo de caracteres por linha de dados adicionais
#DEFINE MAXMSG    009                                                // Máximo de dados adicionais por página
...
Local aArea     := GetArea()
Local lExistNfe := .F.
Private  SPEDTRA:= Upper(AllTrim(GetNewPar("ES_SPEDTRA","")))//RBORGES 13/11/2013 - Usuários que podem imprimir NFE
...
/*==================================================================
+ RBORGES - 18/11/2013 -  							               +
+ DESC.:Gravar no Kardex a cada danfe impresso.                    +
==================================================================*/

xImpDanfe(MV_PAR01,MV_PAR02,MV_PAR03)
...
dbselectarea("TRBSF3")
Do While TRBSF3->(!Eof())
	cNFiscal := TRBSF3->F3_NFISCAL
	cSerie   := TRBSF3->F3_SERIE
	cCliente := TRBSF3->F3_CLIEFOR
	cLoja    := TRBSF3->F3_LOJA
	
	SF2->(DbSetOrder(1))
	If SF2->(DbSeek(xFilial("SF2")+cNFiscal+cSerie+cCliente+cLoja)) .And. !Empty(SF2->F2_CHVNFE)
		
		If !("TMSA200" $ FUNNAME())
			SD2->(DbSetOrder(3))
			If SD2->(DbSeek(xFilial("SD2")+cNFiscal+cSerie+cCliente+cLoja))
				U_DiprKardex(SD2->D2_PEDIDO,U_DipUsr(),"IMPRESSAO DO DANFE "+cNFiscal,.T.,"58") // RBORGES - 14/11/2013
			EndIf
		EndIf
	EndIf
	TRBSF3->(dbSkip())
					...
					If !Empty(aXML[nX][2])
						...
						If (!Empty(cAutoriza) .Or. !Empty(cCodAutDPEC) .Or. Alltrim(aXML[nX][8]) $ "2,5") .And. !cCodRetNFE $ RetCodDene()
							...
							Else
								...
								/*
								-----------------------------------------------------------------------------------------
								REGINALDO BORGES 13/11/2013 - If !(Upper(U_DipUsr()) $ SPEDTRA) .AND. SF2->F2_FIMP <>"S"
								Objetivo......: Restringir a primeira impressão do danfe, liberado apenas ao faturamento.
								-----------------------------------------------------------------------------------------
								*/
								If !(Upper(U_DipUsr()) $ SPEDTRA) .AND. SF2->F2_FIMP <>"S"
									Alert(Upper(U_DipUsr())+", Processo do CD ainda não foi concluído, tente imprimir mais tarde!","Atenção")
									Return()                     
								EndIF 
...
//-------------------------------------------------------------------------------------
// JBS 08/02/2011 - Fontes customizadas para Dipromed
//-------------------------------------------------------------------------------------
//PRIVATE oFont12a  := TFontEx():New(oDanfe,"Courier New"     ,11,08,.F.,.T.,.F.)// 10
//PRIVATE oFont12b  := TFontEx():New(oDanfe,"Times New Roman" ,11,07,.F.,.T.,.F.)// 10
//                                   oDanfe,cName            ,nWidth,nHeight,lBold,lUnderline,lItalic

PRIVATE oFont12a  := TFontEx():New(oDanfe,"Courier New"      ,09    ,07     ,.F.  ,.T.       ,.F.)// 10
PRIVATE oFont12b  := TFontEx():New(oDanfe,"Times New Roman"  ,09    ,06     ,.T.  ,.T.       ,.F.)// 10
PRIVATE oFont12aN := TFontEx():New(oDanfe,"Courier New"      ,09    ,07     ,.T.  ,.T.       ,.F.)// 10
PRIVATE oFont10a  := TFontEx():New(oDanfe,"Times New Roman"  ,09    ,06     ,.F.  ,.T.       ,.F.)// 10
...
Local cChaveCont 	:= ""
Local cLogo01    := FisxLogo("1")
Local cLogo02    := FisxLogo("2")
Local cLogo03    := FisxLogo("3")
Local cGuarda       := ""  
Local cEsp		    := "" 
Local cLogoD	    := ""
local cEndDest      := ""
...
Local nCont			:= 0 //Rafael Moraes Rosa - 26/01/2023
...
For nZ := 1 To nLenDet
			...
			For nY := 1 To nLenSit
				...
				If Type("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2])<>"U"
					...
					If Type("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nPrivate2]+":_VBCST:TEXT")<>"U"
						nBaseST  := Val(&("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nY]+":_VBCST:TEXT"))
						nValST	 := Val(&("oImposto:_Imposto:_ICMS:_ICMS"+aSitTrib[nY]+":_VICMSST:TEXT"))
					EndIf
	...
	nLen     := Len(aItens)
	cAuxItem := AllTrim(SubStr(NoChar(oDet[nX]:_Prod:_xProd:TEXT,lConverte),(nMaxDes+1)))
	cAux 	 :=  AllTrim(NoChar(oDet[nX]:_Prod:_xProd:TEXT+IIf(Type("oDet["+Str(nX)+"]:_InfAdProd:TEXT")<>"U"," "+oDet[nX]:_InfAdProd:TEXT,""),lConverte))

	aadd(aItens,{SubStr(oDet[nX]:_Prod:_cProd:TEXT,1,nMaxCod),;                                            // Codigo do Produto
	SubStr(cAux,1,MAXITEMC),;                       // Descricao do produto
	IIF(Type("oDet[nPrivate]:_Prod:_NCM")=="U","",oDet[nX]:_Prod:_NCM:TEXT),;                                            //  NCM/SH
	cSitTrib,;                                                                                   // CST
	oDet[nX]:_Prod:_CFOP:TEXT,;                                                                 // CFO
	IIf(cEmpAnt=='04',TransForm(nQtd,"@E 999,999.99"),TransForm(nQtd    ,"@E 9,999,999"))+"  "+AllTrim(oDet[nX]:_Prod:_utrib:TEXT),;                                                        // Quantidade
	AllTrim(TransForm(nVUnit  ,"@E 9,999,999.9999")),;                                                   // Valor Unitario
	AllTrim(TransForm(nVTotal ,"@E 99,999,999.99")),;                                                    // Valor Total
	AllTrim(TransForm(nValICM ,"@E 99,999,999.99")),;                                                    // Valor ICMS
	AllTrim(TransForm(nPICM   ,"@E 99.9")),;                                                             // Aliquota do ICMS
	AllTrim(TransForm(nBaseST ,"@E 99,999,999.99"))})                                                    // Base de Calculo Substituicao tributaria

	cAux := SubStr(cAux,(MAXITEMC + 1))
	
	//Rafael Moraes Rosa - 26/01/2023 - INICIO - LINHA COMENTADA
	/*
	Do While !Empty(cAux)
		
		aadd(aItens,{"",;                       // 01
		SubStr(cAux,1,MAXITEMC),;   // 02
		"",;                         // 03
		"",;                         // 04
		"",;                         // 05
		"",;                         // 06
		...
		"",;                         // 09
		"",;                         // 10
		""})                         // 11  Substituicao tributaria
		
		cAux := SubStr(cAux,(MAXITEMC + 1))
		
	EndDo
	*/
	//Rafael Moraes Rosa - 26/01/2023 - FIM - LINHA COMENTADA

	//Rafael Moraes Rosa - 26/01/2023 - INICIO - LINHA SUBSTITUIDA
	nCont	:= 1
	
	Do While !Empty(cAux)

		IF nCont = 1
			aadd(aItens,{"",;                                   // Codigo do Produto
			SubStr(cAux,1,MAXITEMC),;  							// Descricao do produto
			"",;                                           		//  NCM/SH
			"",;                                            	//
			"",;                                            	//
			"",; 												// Unidade de Medida
			"",;                                            	//
			AllTrim(TransForm(nBaseICM ,"@E 99,999,999.99")),;  // Base do ICMS
			AllTrim(TransForm(nValIPI  ,"@E 9,999,999.99")),;   // Valor do IPI
			AllTrim(TransForm(nPIPI    ,"@E 99.9")),;           // Aliquota do IPI
			AllTrim(TransForm(nValST   ,"@E 99,999,999.99"))})  // Valor ICMS Substituicao tributaria
	...
	"",;                                                    // Valor ICMS
	"",;                                                             // Aliquota do ICMS
	""})                                                    // Base de Calculo Substituicao tributaria
...
If Type("oNF:_InfNfe:_infAdic:_infCpl:TEXT")<>"U"
	...
	While !Empty(cAux)
		aadd(aMensagem,StrTran(SubStr(cAux,1,IIf(At('|',SubStr(cAux,1,MAXMENLIN)) > 1,At('|',SubStr(cAux,1,MAXMENLIN)) - 1, MAXMENLIN)),'|',''))
		cAux := SubStr(cAux,IIf(At('|',SubStr(cAux,1,MAXMENLIN)) > 1, At('|',SubStr(cAux,1,MAXMENLIN)), MAXMENLIN) + 1)
	EndDo
EndIf
...
aAdd(aMensagem,"Favor conferir sua mercadoria no ato da entrega, não nos responsabilizamos por reclamações posteriores.")
...
//-- ***********************************************************************
//-- Daniel Leme - 26/04/2011: Novo cálculo de Qtde de Folhas
//-- ***********************************************************************
nNewFolhas := 1

//-- Verifica se haverá mais de uma folha com Itens
If Len(aItens) > MAXITEM
	nNewFolhas += (Len(aItens)-MAXITEM) / MAXITEMP2F
	nNewFolhas := Int(nNewFolhas) + Iif( Int(nNewFolhas) != nNewFolhas, 1, 0)
EndIf

//-- Verifica se haverá mais de uma folha para as mensagens
If Len(aMensagem) > MAXMSG
	nNewFolhas += (Len(aMensagem)-MAXMSG) / MAXMENLIN
	nNewFolhas := Int(nNewFolhas) + Iif( Int(nNewFolhas) != nNewFolhas, 1, 0)
EndIf
//-- ***********************************************************************
//-- Daniel Leme - 26/04/2011: Fim do  novo cálculo
//-- ***********************************************************************
...
EndDo
//-- ***********************************************************************
//-- Daniel Leme - 26/04/2011: Comparação e Log do novo cálculo de Qtde de Folhas
//-- ***********************************************************************
If nFolhas != nNewFolhas
	DanfeIILog("DANFE nº "+StrZero(Val(oIdent:_NNf:Text),9)+ " - Diferença Total Folhas: ANTES: " + AllTrim(Str(nFolhas)) + ", DEPOIS: " + AllTrim(Str(nNewFolhas)) + " (Assumido como correto) ")
	nFolhas := nNewFolhas
EndIf
//-- ***********************************************************************
//-- Daniel Leme - 26/04/2011: Fim da Comparação e Log
//-- ***********************************************************************
...
oDanfe:Say(052,108, "Identificação do emitente",oFont12b:oFont) // JBS 21/02/2011 - oFont12N
nLinCalc	:=	090
...
For nX := 1 To nForTo
	oDanfe:Say(nLinCalc,010,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*50)+1),50), oFont12n:oFont )   // JBS 21/02/2011
	nLinCalc+=07
Next nX
...
nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
For nX := 1 To nForTo
	oDanfe:Say(nLinCalc,010,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*60)+1),60),oFont08N:oFont)
	nLinCalc+=07
Next nX
...
If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
	...
	nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
	For nX := 1 To nForTo
		oDanfe:Say(nLinCalc,010,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*50)+1),50),oFont08N:oFont)
		nLinCalc+=07
	Next nX
	oDanfe:Say(nLinCalc,010, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text,oFont08N:oFont)
	nLinCalc+=07
	oDanfe:Say(nLinCalc,010, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
Else
	oDanfe:Say(nLinCalc,010, NoChar(oEmitente:_EnderEmit:_xBairro:Text,lConverte)+" Cep:"+TransForm(IIF(Type("oEmitente:_EnderEmit:_Cep")=="U","",oEmitente:_EnderEmit:_Cep:Text),"@r 99999-999"),oFont08N:oFont)
	nLinCalc+=07
	oDanfe:Say(nLinCalc,010, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text,oFont08N:oFont)
	nLinCalc+=07
	oDanfe:Say(nLinCalc,010, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
...
If nfolha==1
	If lMv_Logod
		oDanfe:SayBitmap(048,010,cLogoD,095,028)
	Else
		oDanfe:SayBitmap(048,010,cLogo01,095,028)
	EndIF
Endif
...
Endif                                                                       
cDataEmi := Iif(oNF:_INFNFE:_VERSAO:TEXT >= "3.10",StrTran(SubStr(oNFe:_NFE:_INFNFE:_IDE:_DHEMI:Text,1,10),"-",""),StrTran(SubStr(oNFe:_NFE:_INFNFE:_IDE:_DEMI:Text,1,10),"-",""))
oDanfe:Say(158,302,IIF(!Empty(cCodAutDPEC),cCodAutDPEC+" "+AllTrim(ConvDate(cDataEmi))+" "+AllTrim(cDtHrRecCab),IIF(!Empty(cCodAutSef) .And. ((Val(oNF:_INFNFE:_IDE:_SERIE:TEXT) >= 900).And.(oNFe:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT)$"23") .Or. (oNFe:_NFE:_INFNFE:_IDE:_TPEMIS:TEXT)$"1",cCodAutSef+" "+AllTrim(ConvDate(cDataEmi))+" "+AllTrim(cDtHrRecCab),TransForm(cChaveCont,"@r 9999 9999 9999 9999 9999 9999 9999 9999 9999"))),oFont08:oFont)
...
//-- Retirado por Daniel Leme - 26/04/2011
// oDanfe:Say(233,504,Iif( Empty(aDest[4]),"",ConvDate(aDest[4]) ),oFont08:oFont)
...
ElseIf cModFrete =="1"
	// JBS 03/03/2011 - oDanfe:Say(382,247,"1-DEST/REM",oFont08:oFont)
	oDanfe:Say(382,247,"1-DESTINARIO",oFont08:oFont)
ElseIf cModFrete =="2"
...
// JBS 12/02/2011 - Na Danfe da Dipromed e HQ
...
//³Dados Adicionais                                                        ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
oDanfe:Say(761,000,"DADOS ADICIONAIS",oFont08N:oFont)
oDanfe:Box(763,000,877,400)
...
If(MV_PAR04==2)
	oDanfe:Say(nLin,002,"Pedido: ",oFont12:oFont)
	oDanfe:Say(nLin,033,cPedido,oFont18n:oFont)
	nLin:= nLin+10
EndIf

nMensagem := 0
For nX := 1 To Min(nLenMensagens, MAXMSG)
	oDanfe:Say(nLin,002,aMensagem[nX],oFont10a:oFont)
	nLin:= nLin+10
Next nX
nMensagem := nX
If cEmpAnt = "01"
	oDanfe:SayBitmap( 764, 342, "\system\CB.bmp"  ,056, 056)  // Giovani Zago 06/01/12 ..Logo de boas praticas de fabricação.
	oDanfe:SayBitmap( 820, 344, "\system\seaa.bmp",055, 055) // Giovani Zago 06/01/12 ..Logo do amigo animal.
ElseIf cEmpAnt = "04"
	oDanfe:SayBitmap( 815, 339, "\system\LOGOBPFHQ.bmp",059, 059) // Giovani Zago 13/01/12 ..Logo de boas praticas de fabricação da HQ.
EndIf

oDanfe:Box(763,400,877,603)// Giovani Zago 13/01/12
oDanfe:Say(771,402,"RESERVADO AO FISCO",oFont08:oFont)

nLenMensagens:= Len(aResFisco)
nLin:= 783
For nX := 1 To Min(nLenMensagens, MAXMSG)
	oDanfe:Say(nLin,402,aResFisco[nX],oFont10a:oFont)
	nLin:= nLin+10
Next
...
oDanfe:Say(440,002,"DADOS DO PRODUTO",oFont08N:oFont)
oDanfe:Box(442,000,754,603)
oDanfe:Box(442,000,754,052)

oDanfe:Box(442,000,462,026)
oDanfe:Box(462,000,754,026)
oDanfe:Say(450,001,"CODIGO",oFont12aN:oFont)

oDanfe:Box(442,026,462,213)
oDanfe:Box(462,026,754,213)
oDanfe:Say(450,033,"DESCRIÇÃO DO PRODUTO",oFont12aN:oFont)

oDanfe:Box(442,213,462,250)
oDanfe:Box(462,213,754,250)
oDanfe:Say(450,219,"NCM/SH",oFont12aN:oFont)

oDanfe:Box(442,250,462,265)
oDanfe:Box(462,250,754,265)
oDanfe:Say(450,252,"CST",oFont12aN:oFont)

oDanfe:Box(442,265,462,290)
oDanfe:Box(462,265,754,290)
oDanfe:Say(450,269,"CFOP",oFont12aN:oFont)

oDanfe:Box(442,290,462,345)
oDanfe:Box(462,290,754,345)
oDanfe:Say(450,300,"QUANTIDADE",oFont12aN:oFont)
oDanfe:Say(458,300,"        UM",oFont12aN:oFont)

oDanfe:Box(442,345,462,402)
oDanfe:Box(462,345,754,402)
oDanfe:Say(450,358,"V.UNITARIO",oFont12aN:oFont)

oDanfe:Box(442,402,462,465)
oDanfe:Box(462,402,754,465)
oDanfe:Say(450,431,"V.TOTAL",oFont12aN:oFont)
oDanfe:Say(458,431,"BC.ICMS",oFont12aN:oFont)

oDanfe:Box(442,465,462,521)
oDanfe:Box(462,465,754,521)
oDanfe:Say(450,492,"V.ICMS",oFont12aN:oFont)
oDanfe:Say(458,492," V.IPI",oFont12aN:oFont)

oDanfe:Box(442,521,462,552)
oDanfe:Box(462,521,754,552)
oDanfe:Say(450,526,"A.ICMS",oFont12aN:oFont)
oDanfe:Say(458,526," A.IPI",oFont12aN:oFont)

oDanfe:Box(442,552,462,605)
oDanfe:Box(462,552,754,605)
oDanfe:Say(450,561,"BC ICMS ST",oFont12aN:oFont)
oDanfe:Say(458,561," V ICMS ST",oFont12aN:oFont)
...
For nY := 1 To nLenItens
	...
	If lflag
		If nL > nMaxItemP2
			...
			Else
				nLinhavers := 0
			EndIf		
			nLinha    	:=	190 + IIF(nFolha >=3 ,0, nLinhavers)
			...
			If lMv_Logod
				oDanfe:SayBitmap(048,002,cLogoD,095,028)
			Else
				oDanfe:SayBitmap(048,010,cLogo01,095,028)
			EndIF
			...
			nColLim	:=	Iif(MV_PAR05==1,435,865)
			
			oDanfe:Say(161,002,"DADOS DO PRODUTO",oFont12a:oFont)
			oDanfe:Box(163,000,nColLim,603)
			
			oDanfe:Box(163,000,183,026)
			oDanfe:Box(183,000,nColLim,026)
			oDanfe:Say(171,002,"CODIGO",oFont12aN:oFont)
			
			oDanfe:Box(163,026,183,213)
			oDanfe:Box(183,026,nColLim,213)
			oDanfe:Say(171,033,"DESCRIÇÃO DO PRODUTO",oFont12aN:oFont)
			
			oDanfe:Box(163,213,183,250)
			oDanfe:Box(183,213,nColLim,250)
			oDanfe:Say(171,219,"NCM/SH",oFont12aN:oFont)
			
			oDanfe:Box(163,250,183,265)
			oDanfe:Box(183,250,nColLim,265)
			oDanfe:Say(171,252,"CST",oFont12aN:oFont)
			
			oDanfe:Box(163,265,183,290)
			oDanfe:Box(183,265,nColLim,290)
			oDanfe:Say(171,269,"CFOP",oFont12aN:oFont)
			
			oDanfe:Box(163,290,183,345)
			oDanfe:Box(183,290,nColLim,345)
			oDanfe:Say(171,300,"QUANTIDADE",oFont12aN:oFont)
			oDanfe:Say(179,300,"        UM",oFont12aN:oFont)
			
			oDanfe:Box(163,345,183,402)
			oDanfe:Box(183,345,nColLim,402)
			oDanfe:Say(171,358,"V.UNITARIO",oFont12aN:oFont)
			
			oDanfe:Box(163,402,183,465)
			oDanfe:Box(183,402,nColLim,465)
			oDanfe:Say(171,431,"V.TOTAL",oFont12aN:oFont)
			oDanfe:Say(179,431,"BC.ICMS",oFont12aN:oFont)
			
			oDanfe:Box(163,465,183,521)
			oDanfe:Box(183,465,nColLim,521)
			oDanfe:Say(171,492,"V.ICMS",oFont12aN:oFont)
			oDanfe:Say(179,492," V.IPI",oFont12aN:oFont)
			
			oDanfe:Box(163,521,183,552)
			oDanfe:Box(183,521,nColLim,552)
			oDanfe:Say(171,526,"A.ICMS",oFont12aN:oFont)
			oDanfe:Say(179,526," A.IPI",oFont12aN:oFont)
			
			oDanfe:Box(163,552,183,605)
			oDanfe:Box(183,552,nColLim,605)
			oDanfe:Say(171,561,"BC ICMS ST",oFont12aN:oFont)
			oDanfe:Say(179,561," V ICMS ST",oFont12aN:oFont)
			...
			IF MV_PAR05 <> 1 .And. nMensagem <= nLenMensagens
				...
				For nX := 1 To Min(nLenMensagens - nMensagem, MAXMSG)
					oDanfe:Say(nLin,002,aMensagem[nMensagem+nX],oFont10a:oFont)
					nLin:= nLin+10
				Next nX
	...
	If	nL > Iif( (nfolha-1)%2==0 .And. MV_PAR05==1,MAXITEMP3,nMaxItemP2)
		...
		oDanfe:Say(010,108, "Identificação do emitente",oFont12b:oFont) // JBS 21/02/2011 - oFont12N
		nLinCalc	:=	023
		cStrAux		:=	AllTrim(NoChar(oEmitente:_xNome:Text,lConverte))
		nForTo		:=	Len(cStrAux)/50
		nForTo		+=	Iif(nForTo>Round(nForTo,0),Round(nForTo,0)+1-nForTo,nForTo)
		...
		//oDanfe:SayBitmap(400,0100,cLogo01,300,90)	// Logo da Marca d'agua
		...
		oDanfe:Say(161,002,"DADOS DO PRODUTO",oFont08N:oFont)
		oDanfe:Box(163,000,nColLim,603)
		/*
		...
		*/
		// Desenho dos dados do produto
		
		/*
		JBS - 12/02/2011
		----------------
		oDanfe:Box(163,000,nColLim,052)
		oDanfe:Say(171,002,"CODIGO",oFont08N:oFont)
		oDanfe:Box(163,052,nColLim,155)
		oDanfe:Say(171,054,"DESCRIÇÃO DO PRODUTO",oFont08N:oFont)
		...
		*/
		...
		oDanfe:Box(163,026,183,210)
		oDanfe:Box(183,026,nColLim,210)
		oDanfe:Say(171,038,"DESCRIÇÃO DO PRODUTO",oFont12aN:oFont)
		
		oDanfe:Box(163,210,183,250)
		oDanfe:Box(183,210,nColLim,250)
		oDanfe:Say(171,210,"NCM/SH",oFont12aN:oFont)
		...
		oDanfe:Box(163,265,183,290)
		oDanfe:Box(183,265,nColLim,290)
		oDanfe:Say(171,267,"CFOP",oFont12aN:oFont)
		
		oDanfe:Box(163,288,183,343)
		oDanfe:Box(183,288,nColLim,343)
		oDanfe:Say(171,298,"QUANTIDADE",oFont12aN:oFont)
		oDanfe:Say(179,298,"        UM",oFont12aN:oFont)
		
		oDanfe:Box(163,343,183,400)
		oDanfe:Box(183,343,nColLim,400)
		oDanfe:Say(171,355,"V.UNITARIO",oFont12aN:oFont)
		
		oDanfe:Box(163,400,183,463)
		oDanfe:Box(183,400,nColLim,463)
		oDanfe:Say(171,410,"V.TOTAL",oFont12aN:oFont)
		oDanfe:Say(179,410,"BC.ICMS",oFont12aN:oFont)
		
		oDanfe:Box(163,463,183,519)
		oDanfe:Box(183,463,nColLim,519)
		oDanfe:Say(171,472,"V.ICMS",oFont12aN:oFont)
		oDanfe:Say(179,472," V.IPI",oFont12aN:oFont)
		
		oDanfe:Box(163,519,183,548)
		oDanfe:Box(183,519,nColLim,548)
		oDanfe:Say(171,522,"A.ICMS",oFont12aN:oFont)
		oDanfe:Say(179,522," A.IPI",oFont12aN:oFont)
		
		oDanfe:Box(163,548,183,603)
		oDanfe:Box(183,548,nColLim,603)
		oDanfe:Say(171,554,"BC ICMS ST",oFont12aN:oFont)
		oDanfe:Say(179,554," V ICMS ST",oFont12aN:oFont)
	...
	oDanfe:Say(nLinha,002,aAux[1][01][nY],oFont08:oFont )
	oDanfe:Say(nLinha,054,aAux[1][02][nY],oFont08:oFont)// DESCRICAO DO PRODUTO
	oDanfe:Say(nLinha,157,aAux[1][03][nY],oFont08:oFont)//NCM
	oDanfe:Say(nLinha,192,aAux[1][04][nY],oFont08:oFont)//CST
	oDanfe:Say(nLinha,212,aAux[1][05][nY],oFont08:oFont)//CFOP
	oDanfe:Say(nLinha,232,aAux[1][06][nY],oFont08:oFont)//UN
	
	oDanfe:SayAlign(nLinha-5,243,aAux[1][07][nY],oFont08:oFont,45, 10, , 1,  )//QUANT
	oDanfe:SayAlign(nLinha-5,293,aAux[1][08][nY],oFont08:oFont,52, 10, , 1,  )//V UNITARIO
	oDanfe:SayAlign(nLinha-5,348,aAux[1][09][nY],oFont08:oFont,50, 10, , 1,  )//V. TOTAL
	...
	oDanfe:Say(nLinha,001,aAux[1][01][nY],oFont12a:oFont )
	oDanfe:Say(nLinha,028,aAux[1][02][nY],oFont12a:oFont) // DESCRICAO DO PRODUTO
	oDanfe:Say(nLinha,215,aAux[1][03][nY],oFont12a:oFont) // NCM
	oDanfe:Say(nLinha,251,aAux[1][04][nY],oFont12a:oFont) // CST
	oDanfe:Say(nLinha,269,aAux[1][05][nY],oFont12a:oFont) // CFOP
	
	oDanfe:SayAlign(nLinha-5,290,alltrim(aAux[1][06][nY]),oFont12a:oFont,45, 10, , 1,  ) // QUANTIDADE / U.M.
	oDanfe:SayAlign(nLinha-5,347,aAux[1][07][nY],oFont12a:oFont,52, 10, , 1,  ) // V UNITARIO
	oDanfe:SayAlign(nLinha-5,410,aAux[1][08][nY],oFont12a:oFont,50, 10, , 1,  ) // V. TOTAL / BC.ICMS
	oDanfe:SayAlign(nLinha-5,470,aAux[1][09][nY],oFont12a:oFont,45, 10, , 1,  ) // V. ICMS / V.IPI
	oDanfe:SayAlign(nLinha-5,519,aAux[1][10][nY],oFont12a:oFont,30, 10, , 1,  ) // A.ICMS  / A.IPI
	oDanfe:SayAlign(nLinha-5,564,aAux[1][11][nY],oFont12a:oFont,36, 10, , 1,  ) // BC ICMS ST / V.ICMS ST
...
//oDanfe:SayBitmap(650,100,cLogo03,100,100)
...
If lMensagens
	...
	For nX := nMensagem To nForMensagens
		oDanfe:Say(nlinha,002,aMensagem[nX],oFont10a:oFont)
		nMensagem++
		nLinha:= nLinha+ 10
	Next nX
EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³DanfeIILogºAutor  ³Microsiga           º Data ³  04/26/11   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ Gravação de Log Especifico. Fara                           º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ DANFE II                                                  º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
*/
Static Function DanfeIILog(_cString)

If dDataBase < StoD("20110515")
	If !File("DANFEII_DIF_"+DtoS(dDataBase)+".LOG")
		nHandle := MsfCreate("DANFEII_DIF_"+DtoS(dDataBase)+".LOG",0)
	Else
		nHandle := FOpen("DANFEII_DIF_"+DtoS(dDataBase)+".LOG",2)
		fSeek(nHandle,0,2)
	EndIf
	If nHandle > 0
...
/*==================================================================
+ RBORGES - 18/11/2013 -  FUNÇÃO: xImpDanfe()                      +
+ DESC.:Retonar o número das nota fiscais de acordo com as informa-+
+ ções no parâmetro.                                               +
==================================================================*/


Static Function xImpDanfe(MV_PAR01,MV_PAR02,MV_PAR03)
...
cSQL :="SELECT"
cSQL +=" DISTINCT F3_NFISCAL, F3_SERIE, F3_CLIEFOR,F3_LOJA"
cSQL +=" FROM "
cSQL +=" 	"+RETSQLNAME("SF3")+""
cSQL +="     WHERE "
cSQL +="           F3_FILIAL   = '"+xFILIAL("SF3")+"' "
cSQL +="       AND F3_SERIE    = '"+MV_PAR03+"' "
cSQL +="       AND F3_NFISCAL >= '"+MV_PAR01+"' "
cSQL +="       AND F3_NFISCAL <= '"+MV_PAR02+"' "
cSQL +="       AND "+RETSQLNAME("SF3")+".D_E_L_E_T_='' "


dbUseArea(.T.,"TOPCONN",TcGenQry(,,cSQL),"TRBSF3",.T.,.T.)
