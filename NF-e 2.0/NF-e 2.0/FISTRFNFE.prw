...
±±ºPrograma  ³FISTRFNFE ºAutor  ³Microsiga           º Data ³  03/31/10   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³PE na montagem do menu do SEFAZ. Utilizado para controle    º±±
±±º          ³adicional de acesso - ES_SPEDROT e ES_SPEDUSR               º±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³SPEDVldUsrºAutor  ³Microsiga           º Data ³  03/31/10   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³PE na montagem do menu do SEFAZ. Utilizado para controle    º±±
±±º          ³adicional de acesso - ES_SPEDROT e ES_SPEDUSR               º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
Local cRotCtl 	:= SuperGetMV("ES_SPEDROT",,"Wiz.Config.|P&arâmetros|Status SEFAZ|Transmissão|DANFE|Monitor|Visualiza Doc.|Filtro|Exportar|Legenda|Faixa|Tempo|Estatísticas|Consulta NFe")
//Local nPosRot	:= 0
//Local nAux		:= 0

//-- Verifica se a opção é controlada
If AllTrim(Upper(cTitulo)) $ AllTrim(Upper(cRotCtl)) .Or. AllTrim(Upper(u_DipRetCE(cTitulo))) $ AllTrim(Upper(u_DipRetCE(cRotCtl)))

/*	nPosRot := 1

	//-- Localiza parametro de permissão utilizado
...
*/
	//-- Caso o Usuário não possua Direitos á Rotina, altera o Título e Chamada
	...
	If !(AllTrim(Upper(cUserName)) $ AllTrim(Upper(SuperGetMV("ES_SPEDUSR",,""))))
		//cRotina := 'MsgAlert("Usuário não Autorizado! Entre em contato com o CPD.","ES_SPEDU'+StrZero(nPosRot,2)+' - '+AllTrim(cTitulo)+'")'
		cRotina := If(ValType(cRotina)=="A","",cRotina)
		cRotina := 'MsgAlert("Usuário não Autorizado! Entre em contato com o CPD.","ES_SPEDUSR - '+AllTrim(cTitulo)+'/'+AllTrim(cRotina)+'")'
		cTitulo := "@ "+AllTrim(cTitulo)
	EndIf
EndIf

//- Libera transmissão para cd - MCVN 29/03/11
If AllTrim(Upper(cTitulo)) $ Upper("|Transmissão|") .Or. AllTrim(Upper(u_DipRetCE(cTitulo))) $ Upper("|Transmissão|")

	If !(AllTrim(Upper(cUserName)) $ AllTrim(Upper(SuperGetMV("ES_SPEDTRA",,""))))
		//cRotina := 'MsgAlert("Usuário não Autorizado! Entre em contato com o CPD.","ES_SPEDU'+StrZero(nPosRot,2)+' - '+AllTrim(cTitulo)+'")'
		cRotina := If(ValType(cRotina)=="A","",cRotina)
		cRotina := 'MsgAlert("Usuário não Autorizado! Entre em contato com o CPD.","ES_SPEDTRA - '+AllTrim(cTitulo)+'/'+AllTrim(cRotina)+'")'
		cTitulo := "@ "+AllTrim(cTitulo)
	EndIf
...
//////////////////////////////////////////////////////////////
//Private _cTopAlias := "PROTHEUS_PROD"          //DEFINIÇAO DO SCHEMA DE AMBIENTE ALIAS
Private _cTopAlias := "DADOSMP10"          //DEFINIÇAO DO SCHEMA DE AMBIENTE ALIAS
