...
IF cEmpAnt+cFilAnt = '0101'
	...
	SM0->(dbSeek("0104"))
		...
		cEstEnt		:= SM0->M0_ESTENT //69 linhas de ajustes
			...
			Else
				...
				If !SF2->F2_TIPO $ "DB" 
					...
					*/
					If !lUsaCliEnt
						...
						If !Empty(AllTrim(SF2->F2_CLIENT)) .And. !Empty(AllTrim(SF2->F2_LOJENT))			
							If Len(aEntrega) > 0											
								...
								If aEntrega[08] <> IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) .And. !lCNPJIgual //aEntrega[08] <> Upper(SA1->A1_EST) //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
								...
								If aEntrega[08] == IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) .OR. lCNPJIgual //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
									SA1->(MsSeek(xFilial("SA1")+SF2->F2_CLIENTE+SF2->F2_LOJA))
								EndIf
							Endif
						Else
					...
					If !Upper(SA1->A1_EST) == "EX"                                                      				
						...
						If !Empty(SA1->A1_INSCRUR) .And. SA1->A1_PESSOA == "F" .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) == "PR"  .And. SA1->A1_EST == "PR" //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
				...
				If !Empty(SF2->F2_TRANSP)
					...
					//unidade da Federação e a inscrição estadual do transportador (CHAMADO-TVMWZL).
						...
						if (IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) == "MG") .and. Empty(SA4->A4_INSEST) .and. (ALLTRIM(Upper(SA4->A4_NOME)) =='REMETENTE' .OR. ALLTRIM(Upper(SA4->A4_NOME)) =='DESTINATARIO') //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
				...
				If MsSeek(cChave)
					...
					elseif ( AllTrim(SF3->F3_CFO) == "5949" .or. AllTrim(SF3->F3_CFO) == "5910" ) .and. cEstEnt == 'SP' /*termos do inciso II do art. 456 do RICMS/ SP  chamado THPXGS*/ //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					If (cAliasSD2)->D2_TIPO == "D" .And. cEstEnt == "PR" .And. (cAliasSD2)->D2_ICMSRET > 0 //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
	          ...
	          // Posiciono na TES do cupom fiscal para pegar a natureza de operação da nf sobre cupom
									...
									If AllTrim(SF1->F1_ESPECIE)=="NFP"
										...
										If SD1->D1_TIPO$"DB"
												...
												*/
												...
												aadd(aNfVincRur,{SD1->D1_EMISSAO,SD1->D1_SERIE,SD1->D1_DOC,SF1->F1_ESPECIE,;
												IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA1->A1_CGC),; 
												IIF(SD1->D1_FORMUL=="S",cEstEnt,SA1->A1_EST),; 
												IIF(SD1->D1_FORMUL=="S",SM0->M0_INSC,SA1->A1_INSCR)})
										...
										Else
											...
											*/
											...
											aadd(aNfVincRur,{SD1->D1_EMISSAO,SD1->D1_SERIE,SD1->D1_DOC,SF1->F1_ESPECIE,;
											IIF(SD1->D1_FORMUL=="S",SM0->M0_CGC,SA2->A2_CGC),; 
											IIF(SD1->D1_FORMUL=="S",cEstEnt,SA2->A2_EST),; 
											IIF(SD1->D1_FORMUL=="S",SM0->M0_INSC,SA2->A2_INSCR)})
											//Rafael Moraes Rosa - 12/01/2023 - FIM - LINHA SUBSTITUIDA
									   		...
									   		If MsSeek(xFilial("SF3")+SFT->FT_CLIEFOR+SFT->FT_LOJA+SFT->FT_NFISCAL+SFT->FT_SERIE)										   		
												...
												If AllTrim(SF3->F3_ESPECIE)=="NFP"
													...
													*/
													...
													aadd(aNfVincRur,{SF3->F3_EMISSAO,SF3->F3_SERIE,SF3->F3_NFISCAL,SF3->F3_ESPECIE,;
														SM0->M0_CGC,cEstEnt,SM0->M0_INSC})
						...
						if !lIcmDevol .And. !("Nota fiscal emitida sem destaque do ICMS" $ cMensCli)
							...
							if cEstEnt == "PR" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
						...
						If SFT->(FieldPos("FT_DS43080")) <> 0 .And. SFT->FT_DS43080 > 0 .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) == "MG" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
						...
						If cEstEnt $ "PR/RJ/RS/" //TAG cBenef buscar o conteúdo da tabela 5.2 no sistema quando for do PR. //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
							...
							If MsSeek(xFilial("CDV") +'S'+PadR('SPED',TamSX3("CDV_ESPECI")[1])+'S'+(cAliasSD2)->(D2_DOC+D2_SERIE+D2_CLIENTE+D2_LOJA+D2_ITEM))
								...
								cCodlan := getCodLan( alltrim(cEstEnt), SF4->F4_SITTRIB, cAmbiente ) //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA

							EndIF
						Else
							cCodlan := ""	
							//If SM0->M0_ESTENT <> "SP" .and. CDA->(ColumnPos("CDA_CODLAN")) > 0 //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
							If cEstEnt <> "SP" .and. CDA->(ColumnPos("CDA_CODLAN")) > 0 //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
								dbSelectArea("CDA")
								dbSetOrder(1)
						...
						While CD2->(!Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
							...
							ElseIf SFT->(FieldPos("FT_TRFICM")) <> 0 .And. SFT->FT_TRFICM <> 0 .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) $ "RS/GO" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
							...
							If SFT->(FieldPos("FT_PR43080")) <> 0 .And. SFT->FT_PR43080 <> 0 .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) == "MG" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
			...
			If SF1->F1_TIPO $ "DB" 
				...
				*/
				...
				If cDevMerc == "S"
					...
					aadd(aDest,IIF(!lEndFis,ConvType(FisGetEnd(SM0->M0_ENDCOB,SM0->M0_ESTCOB)[1]),ConvType(FisGetEnd(cEndEnt,cEstEnt)[1])))// 3 //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
					...
					Else
						...
						*/
						...
						//Rafael Moraes Rosa - 12/01/2023 - INICIO - LINHA SUBSTITUIDA
						If FisGetEnd(cEndEnt,cEstEnt)[2] <> 0
							aadd(aDest,FisGetEnd(cEndEnt,cEstEnt)[3])// 4
						//Rafael Moraes Rosa - 12/01/2023 - FIM - LINHA SUBSTITUIDA
					...
					*/
					...
					//Rafael Moraes Rosa - 12/01/2023 - INICIO - LINHA SUBSTITUIDA
					cEndEmit :=  IIF(!lEndFis,Iif(!Empty(SM0->M0_COMPCOB),SM0->M0_COMPCOB,ConvType(FisGetEnd(SM0->M0_ENDCOB,SM0->M0_ESTCOB)[4]) ) ,Iif(!Empty(cCompEnt),cCompEnt,ConvType(FisGetEnd(cEndEnt,cEstEnt)[4]) ) )
					aadd(aDest,cEndEmit)// 5
	
					aadd(aDest,IIF(!lEndFis,ConvType(SM0->M0_BAIRCOB),ConvType(cBairEnt)))// 6
					
					aadd(aDest,ConvType(SM0->M0_CODMUN))// 7
					aadd(aDest,IIF(!lEndFis,ConvType(SM0->M0_CIDCOB),ConvType(cCidEnt)))// 8				
					
					aadd(aDest,Upper(IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt))))// 9
					aadd(aDest,IIF(!lEndFis,ConvType(SM0->M0_CEPCOB),ConvType(cCEPEnt)))// 10
					//Rafael Moraes Rosa - 12/01/2023 - FIM - LINHA SUBSTITUIDA
					...
					If !lUsaCliEnt
						...
						If !Empty(AllTrim(SF1->F1_FORENT)) .And. !Empty(AllTrim(SF1->F1_LOJAENT))			
							If Len(aEntrega) > 0											
								...
								If aEntrega[08] <> IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) .And. !lCNPJIgual //aEntrega[08] <> Upper(SA1->A1_EST) //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
								...
								If aEntrega[08] == IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) .OR. lCNPJIgual //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
									MsSeek(xFilial("SA1")+SF1->F1_FORNECE+SF1->F1_LOJA)
								EndIf
							Endif
						Else
...
If cVeramb >= "3.10"
	...
	If cOper == "3" .And. Len(aComb) > 0 .And. aDest[9] != "EX" .And. aComb[1][4] == "EX"		
		...
		If aDest[9] == IIF(!GetNewPar("MV_SPEDEND",.F.),ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
			cOper:= "1" 
		Else
			cOper:= "2"
		EndIf		
...
cString += '<Lgr>'+IIF(!lEndFis,ConvType(FisGetEnd(SM0->M0_ENDCOB,SM0->M0_ESTCOB)[1]),ConvType(FisGetEnd(cEndEnt,cEstEnt)[1]))+'</Lgr>' //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
...
Else
	...
	*/
	...
	//Rafael Moraes Rosa - 12/01/2023 - INICIO - LINHA SUBSTITUIDA
	If FisGetEnd(cEndEnt,cEstEnt)[2]<>0
		cString += '<nro>'+FisGetEnd(cEndEnt,cEstEnt)[3]+'</nro>'
	//Rafael Moraes Rosa - 12/01/2023 - FIM - LINHA SUBSTITUIDA
...
cEndEmit :=  IIF(!lEndFis,Iif(!Empty(SM0->M0_COMPCOB),SM0->M0_COMPCOB,ConvType(FisGetEnd(SM0->M0_ENDCOB,SM0->M0_ESTCOB)[4]) ) , Iif(!Empty(cCompEnt),cCompEnt,ConvType(FisGetEnd(cEndEnt,cEstEnt)[4]) ) )
cString += NfeTag('<Cpl>',cEndEmit)
cString += '<Bairro>'+IIF(!lEndFis,ConvType(SM0->M0_BAIRCOB),ConvType(cBairEnt))+'</Bairro>'
cString += '<cMun>'+ConvType(SM0->M0_CODMUN)+'</cMun>'
cString += '<Mun>'+IIF(!lEndFis,ConvType(SM0->M0_CIDCOB),ConvType(cCidEnt))+'</Mun>'
cString += '<UF>'+IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt))+'</UF>'
cString += NfeTag('<CEP>',IIF(!lEndFis,ConvType(SM0->M0_CEPCOB),ConvType(cCEPEnt)))
//Rafael Moraes Rosa - 12/01/2023 - FIM - LINHA SUBSTITUIDA
...
If !Empty(aIEST) 
	...
	*/
	...
	//Rafael Moraes Rosa - 13/01/2023 - INICIO - LINHA SUBSTITUIDA
	If((AllTrim(ConvType(VldIE(SM0->M0_INSC))) <> Alltrim(aIEST[01])) .And. (Alltrim (aIEST[01]) <> Alltrim(aIEST[02])) .And.;
			 (cIdDest == "2" .OR. (cIdDest == "1" .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) == "PR")))
	//Rafael Moraes Rosa - 13/01/2023 - FIM - LINHA SUBSTITUIDA
...
Static Function NfeDest(aDest,cVerAmb,aTransp,aCST,lBrinde,cMunDest)
	...
	cString += '<Lgr>'+ConvType(iIf(!lBrinde,aDest[03],(FisGetEnd(cEndEnt,cEstEnt)[1])))+'</Lgr>' //Rafael Moraes Rosa - 12/01/2023 - LINHA SUBSTITUIDA
	...
	*/
	...
	cString += NfeTag('<Cpl>',ConvType(iIf(!lBrinde,aDest[05],cCompEnt)))
	cString += '<Bairro>'+ConvType(iIf(!lBrinde,aDest[06],cBairEnt))+'</Bairro>'
	cString += '<cMun>'+ConvType(cMunDest)+'</cMun>'
	cString += '<Mun>'+ConvType(iIf(!lBrinde,aDest[08],cCidEnt))+'</Mun>'
	cString += '<UF>'+ConvType(iIf(!lBrinde,aDest[09],cEstEnt))+'</UF>'
	cString += NfeTag('<CEP>',iIf(!lBrinde,aDest[10],cCEPEnt))
	...
	Else
		...
		If Len(aIcms)>0
			...
			ElseIf ( aCST[1] == '51' .And. lArt186 )
				...
				If !Empty(aICMS[12]) .and. !Empty(aICMS[07]) .and. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) $ "RS" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
			...
			If cVerAmb >= "3.10" .and. aCST[1] $ '10' .and. (IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) == "PR") .and. !Empty(aICMS[12]) //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
	...
	ELse
		...
		If (aCST[01] = "60" .or. cCsosn$ "500")		
			...
			If nBaseIcm > 0 .and. nValICM > 0	.and. nAlqICM > 0
				...
				If cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) == "SP" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
				...
				ElseIF cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) == "PR" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
				...
				ElseIF cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) == "SC" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA  
				...
				ElseIF cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) == "AM" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
				 	lNContrICM := IIf(Empty(SA1->A1_INSCR) .Or. "ISENT"$SA1->A1_INSCR .Or. "RG"$SA1->A1_INSCR .Or. ( SA1->(FieldPos("A1_CONTRIB"))>0 .And. SA1->A1_CONTRIB == "2"),.T.,.F.)
				...
				ElseIF cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) == "RS" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA		 
				 	if !Empty(aProd[28])				 	
				...
				ElseIf cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) == "ES" .And. Len(aICMS) > 0 .And. ( nBaseIcm+nValICM > 0 ) //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
				...
				ElseIf cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) == "MG" .And. Len(aICMS) > 0 .And. ( nBaseIcm+nValICM > 0 )  // Conforme Chamado TIABCS //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
				...
				ElseIf IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) == "SP" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA 
					If cIndFinal == "1"
				...
				ElseIf IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) == "RJ" //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
...
If nIcmsDif > 0 .And. aDest[9] == 'PR' .And. aCST[1] $ '10' .And. cEstEnt == 'PR' //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
...
If Len(cMsgCli)>0 .and. !Empty(cMsgCli)
	...
	IF (cEstEnt) $ "SP" .AND. cTipo=='0' .And. !Empty(cSerRefcup + cNfRefcup) //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
		SFT->(dbSetOrder(6))
...
if cEstEnt == 'SP' .and. Len(aNfVinc) > 0 //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
	...
	For nZ := 1  to Len( aNfVinc )
		If !( aNfVinc[nZ][2] + aNfVinc[nZ][3] ) $ cChvNFe
			...
			Else
				...
				if cEstEnt == 'SP' //Rafael Moraes Rosa - 13/01/2023 - LINHA COMENTADA
			...
			if cEstEnt == 'SP' //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA 
...
If nValII > 0
	...
	If lEasy .And. IIF(!GetNewPar("MV_SPEDEND",.F.),ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) == "SP" .and. lEIC0064 //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA
...
If nvFCPUFDest > 0 .or.  nvICMSUFDest > 0 .or. nvICMSUFRemet > 0 .or. nvBCUFDest  > 0     
	...
	IF (IIF(!(GetNewPar("MV_SPEDEND",.F.)),ConvType(SM0->M0_ESTCOB),ConvType(cEstEnt)) == "BA" ) //Rafael Moraes Rosa - 13/01/2023 - LINHA SUBSTITUIDA  
