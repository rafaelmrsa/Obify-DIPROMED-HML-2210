...
If cFilAnt <> '04'
	Aviso("Aten็ใo","Esta rotina s๓ pode ser executada na filial CD",{"Ok"},3)
	lRet := .F.
...
For nx := 1 to Len(aProc)
	...
	cQuery := " SELECT ISNULL(SUM(B9_QINI),0) AS B9_QINI, ISNULL(SUM(B9_VINI1),0) AS B9_VINI1 " + CRLF 
	...
	cQuery += " AND B9_FILIAL = '01' AND B9_COD = '" + aProc[nx][3] + "' AND B9_LOCAL = '01'  " + CRLF 
	...
	If ( ( _cAlias )->( !Eof() ) )
		...
		While ( ( _c1Alias )->( !Eof() ) )
			...
			If ( _cAlias )->B9_QINI > 0
				nVal := ( _cAlias )->B9_QINI 
			ElseIf ( _cAlias )->B9_VINI1 > 0
				nVal := ( _cAlias )->B9_VINI1 *( _c1Alias )->D3_QUANT
			Else
				nVal := 0
			EndIf
			...
			nQueryRet := TcSqlExec("UPDATE " + RetSqlName("SD3") + " SET D3_CUSTO1 = " + Alltrim(Str(nVal)) + " , D3_CF = '" + cCF + "' WHERE D3_FILIAL = '04' AND R_E_C_N_O_ =  " + cValtoChar(( _c1Alias )->RECSD3))
