...
EndIf
//-----------------------------------------------------
If MV_PAR01+MV_PAR02 == '01105001' //HQ
	ehHQ := .T.
EndIf
//-----------------------------------------------------
...
//CHAMADA DA CONEXAO COM HQ NA NUVEM
If ehHQ
	// Cria uma conexใo com um outro banco
...
// Caso buscar na HQ valida็ใo sera posterior no Array
//----------------------------------------------------------------------------------
if !ehHQ
...
Do While TRB->( !EOF() )
        DbSelectArea("DTC")
        DbSetOrder(2)
        If !DbSeek(xFilial("DTC")+TRB->F2_DOC+TRB->F2_SERIE+MV_PAR01+MV_PAR02)
...
Else	
	...
	Do While SM0->( !EOF() )
		If AllTrim(SM0->M0_CGC) == AllTrim(SA1->A1_CGC)
			...
			Exit
		/* NAO USA MAIS ESSA EMPRESA 0404
